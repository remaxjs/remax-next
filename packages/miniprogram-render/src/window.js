...
class Window extends EventTarget {
  constructor(pageId) {
    ...
    if (config.generate && config.generate.worker) {
      this.$_workerConstructor = class Worker extends WorkerImpl.Worker {
        constructor(url) {
          super(url, pageId);
        }
      };
      this.$_sharedWorkerConstructor = class SharedWorker extends WorkerImpl.SharedWorker {
        constructor(url) {
          super(url, pageId);
        }
      };
  ...
  $$getComputedStyle(dom, computedStyle = []) {
    ...
    return new Promise((resolve, reject) => {
      if (dom.tagName === 'BODY') {
        ...
        this.$$createSelectorQuery()
          .select(`.miniprogram-root >>> .node-${dom.$$nodeId}`)
          .fields({ computedStyle }, res => (res ? resolve(res) : reject()))
          .exec();
      }
    });
