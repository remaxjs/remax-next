...
const WX_COMPONENT_TRANSFORM_LIST = [
  'checkbox',
  'checkbox-group',
  'input',
  'label',
  ...
  $$createElement(options, tree) {
    ...
    if (constructorClass) {
      ...
      if (behavior && WX_COMPONENT_TRANSFORM_LIST.indexOf(behavior) !== -1)
        return transformWxComponent2Dom(behavior, options, tree); // 需要转成普通 dom
      ...
      if (WX_COMPONENT_TRANSFORM_LIST.indexOf(wxComponentName) !== -1)
        return transformWxComponent2Dom(wxComponentName, options, tree); // 需要转成普通 dom
      else {
