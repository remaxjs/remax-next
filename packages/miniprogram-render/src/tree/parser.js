...
const voidMap = {};
['area', 'base', 'br', 'col', 'embed', 'hr', 'img', 'input', 'link', 'meta', 'param', 'source', 'track', 'wbr'].forEach(
  n => (voidMap[n] = true)
);
  ...
  while (content) {
    ...
    if (!stack.last() || !rawTextMap[stack.last()]) {
      ...
      if (isText) {
        ...
        if (
          indexStart === -1 ||
          (indexStart >= 0 && indexEnd === -1) ||
          indexStart > indexEnd ||
          (indexEnd > indexStart && !content.substring(indexStart + 1, indexEnd).trim())
        ) {
    ...
    if (handler.start) {
      ...
      try {
        ...
        rest.replace(attrReg, (all, $1, $2, $3, $4) => {
          ...
          attrs.push({
            name: $1,
            value: value.replace(/url\(:#\|\d+\|#:\)/gi, all => longAttributeCache[all] || 'url()'),
          });
        });
      }
