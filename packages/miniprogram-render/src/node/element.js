...
class Element extends Node {
  ...
  $_generateDomTree(node) {
    const { type, tagName = '', attrs = [], children = [], content = '' } = node;
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.tagName === 'BODY') {
        window
          .$$createSelectorQuery()
          .selectViewport()
          .scrollOffset(res => (res ? resolve(res) : reject(new Error('body not found in webview'))))
          .exec();
      } else {
        window
          .$$createSelectorQuery()
          .select(`.miniprogram-root >>> .node-${this.$_nodeId}`)
          .boundingClientRect(res => (res ? resolve(res) : reject(new Error('element not found in webview'))))
          .exec();
      }
    });
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.tagName === 'CANVAS') {
        ...
        wx.createSelectorQuery()
          ...
          .context(res => (res && res.context ? resolve(res.context) : reject()))
          .exec();
      } else {
        window
          .$$createSelectorQuery()
