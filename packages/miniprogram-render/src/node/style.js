...
function parse(styleText) {
  ...
  if (styleText) {
    ...
    styleText = styleText.replace(/url\([^)]+\)/gi, all => all.replace(/;/gi, ':#||#:')); // 先处理值里面的分号
...
class Style {
      ...
      properties[name] = {
        ...
        set(value) {
          ...
          if (value && config.optimization.styleValueReduce && value.length > config.optimization.styleValueReduce) {
            console.warn(
              `property "${name}" will be deleted, because it's greater than ${config.optimization.styleValueReduce}`
            );
            value = undefined;
          }
  ...
  get cssText() {
    ...
    if (this.$__vars) {
      ...
      const varsText = Object.keys(vars)
