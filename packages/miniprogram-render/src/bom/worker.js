...
class Worker extends EventTarget {
  constructor(url, pageId, isSharedWorker) {
    ...
    if (wxWorker) {
      ...
      if (window && !isSharedWorker) {
        ['userAgent', 'appCodeName', 'appName', 'language', 'languages', 'platform', 'product'].forEach(
          key => (navigator[key] = window.navigator[key])
        );
        ['protocol', 'host', 'hostname', 'port', 'origin', 'pathname', 'search', 'hash', 'href'].forEach(
          key => (location[key] = window.location[key])
        );
      }
      this.$_tryCatch(() =>
        wxWorker.postMessage({
          type: 'connect',
          pageId: this.$_pageId,
          navigator,
          location,
  ...
  postMessage(data) {
    if (this.$_pageId && wxWorker)
      this.$_tryCatch(() => wxWorker.postMessage({ type: 'message', pageId: this.$_pageId, data }));
  }
