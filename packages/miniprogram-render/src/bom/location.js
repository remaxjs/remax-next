...
class Location extends EventTarget {
  ...
  $_setHrefWithoutEnterHistory(value) {
    ...
    const { protocol, hostname, port, hash, search, pathname } = Location.$$parse(value);
  ...
  $$open(url) {
    ...
    if (
      parseRes.protocol !== this.$_protocol ||
      parseRes.hostname !== this.$_hostname ||
      parseRes.port !== this.$_port
    ) {
   ...
   */
  $$reset(url = '') {
    const { protocol, hostname, port, pathname, hash, search } = Location.$$parse(url);
  ...
  $$endCheckHash(needCheckUrlChange) {
    ...
    if (
      (needCheckUrlChange || (this.$_lastPathname === this.$_pathname && this.$_lastSearch === this.$_search)) &&
      this.$_lastHash !== this.$_hash
    ) {
