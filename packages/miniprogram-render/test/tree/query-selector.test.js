...
test('query-selector: parse selector', () => {
  ...
  // 内置组件
  expect(querySelector.parse('#id wx-xxx')).toEqual([{ tag: '*', id: 'id', kinship: ' ' }, { tag: 'wx-xxx' }]);
...
test('query-selector: exec select', () => {
  ...
  expect(querySelector.exec('.bb footer > .bb4:nth-child(2)', getExtra(document))[0]).toBe(
    document.body.querySelector('.bb footer > .bb4:nth-child(2)')
  );
  expect(querySelector.exec('.bb footer > .bb4:first-child', getExtra(document))[0]).toBe(
    document.body.querySelector('.bb footer > .bb4:first-child')
  );
  expect(querySelector.exec('.bb footer > .bb4:last-child', getExtra(document))[0]).toBe(
    document.body.querySelector('.bb footer > .bb4:last-child')
  );
  ...
  expect(querySelector.exec('.bb wx-view', getExtra(document))[0]).toBe(
    document.body.querySelector('.bb wx-component[behavior=view]')
  );
  expect(querySelector.exec('.bb wx-text', getExtra(document))[0]).toBe(
    document.body.querySelector('.bb wx-component[behavior=text]')
  );
});
