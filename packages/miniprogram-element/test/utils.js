...
function getData(component) {
  return (
    (component.data.childNodes && component.data.childNodes[0] && component.data.childNodes[0].extra) || component.data
  );
}
...
simulate.checkEvent = async function (component, node, eventNameList) {
  ...
  expect(evtList.map(evt => evt.type)).toEqual(
    eventNameList.map(eventName => (Array.isArray(eventName) ? eventName[1] : eventName))
  );
};
