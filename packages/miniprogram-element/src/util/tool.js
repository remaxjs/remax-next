...
const { cache, tool } = mp.$$adapter;

const { wxCompData, wxCompNameMap, wxSubComponentMap } = component;
...
function filterNodes(domNode, level, component) {
  ...
  return childNodes
    .map(child => {
      ...
      domInfo.className =
        domInfo.type === 'element' ? `h5-${domInfo.tagName} node-${domInfo.nodeId} ${domInfo.className || ''}` : ''; // 增加默认 class
      ...
      if (domInfo.useTemplate) {
        ...
        if (wxCompName)
          checkComponentAttr(
            ...
            extra,
            null,
            `h5-${domInfo.tagName} ${domInfo.tagName === 'wx-component' ? 'wx-' + child.behavior : ''}`
          );
        ...
        if (relationIndex !== -1) {
          ...
          extra.childNodes = childNodes
            .filter(childNode => childNode.type === 'element' && childNode.compName === RELATION_CHILD[relationIndex])
            .map(childNode => {
              ...
              if (copyChildNode.childNodes) {
                ...
                copyChildNode.childNodes = copyChildNode.childNodes.map(grandchild =>
                  Object.assign({}, grandchild, { style: '' })
                );
              }
          ...
          const slots = child.childNodes
            .map(childNode => {
              ...
              return {
                ...
                id: slotDomInfo.id,
                className:
                  slotDomInfo.type === 'element'
                    ? `h5-${slotDomInfo.tagName} node-${slotDomInfo.nodeId} ${slotDomInfo.className || ''}`
                    : '',
    ...
    if (newIsArray && oldIsArray) {
      ...
      if (newIsNode && oldIsNode && newItemType === oldItemType) {
        ...
        for (const key of keys) {
          ...
          const isInterrupt = getDiffChildNodes(
            newItem[key],
            oldItem[key],
            destData,
            dataPath,
        ...
        if (isExtra && newItem.forceUpdate) {
          ...
          for (const key of newKeys) {
            ...
            if (!hasOwnProperty.call(oldItem, key)) {
              ...
              if (domNode && domNode._oldValues && domNode._oldValues[key] !== undefined)
                oldItemValue = domNode._oldValues[key];
