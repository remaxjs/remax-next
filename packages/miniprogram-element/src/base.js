...
const { cache, Event, EventTarget, tool } = mp.$$adapter;
const { USE_TEMPLATE } = _;
const { wxCompHandles, wxCompNameMap } = component;
      ...
      EventTarget.$$process(domNode, eventName, evt, extra, (domNode, evt, isCapture) => {
        ...
        setTimeout(() => {
          ...
          if (domNode.tagName === 'A' && evt.type === 'click' && !isCapture) {
              ...
              if (type === 'radio') {
                ...
                if (name) {
                  this.callSimpleEvent(
                    'change',
                    { detail: { value: targetDomNode.checked ? [targetDomNode.value] : [] } },
                    targetDomNode
                  );
                } else {
                  const target = { dataset: { pageId: this.pageId, privateNodeId: targetDomNode.$$nodeId } };
            ...
            (domNode.tagName === 'BUTTON' || (domNode.tagName === 'WX-COMPONENT' && domNode.behavior === 'button')) &&
            evt.type === 'click' &&
            !isCapture
          ) {
            // 处理 button 点击
            const type =
              domNode.tagName === 'BUTTON' ? domNode.getAttribute('type') : domNode.getAttribute('form-type');
            ...
            const pickerList = form
              ...
              .filter(item => !!item.getAttribute('name'));
            const pickerViewList = form
              .querySelectorAll('wx-component[behavior=picker-view]')
              .filter(item => !!item.getAttribute('name'));
            ...
            if (type === 'submit') {
              ...
              if (switchList.length)
                switchList.forEach(item => (formData[item.getAttribute('name')] = !!item.getAttribute('checked')));
              if (sliderList.length)
                sliderList.forEach(item => (formData[item.getAttribute('name')] = +item.getAttribute('value') || 0));
